...
double NonUniformBspline::getAcc() {
  NonUniformBspline acc_traj = getDerivative().getDerivative();

  Eigen::VectorXd times     = acc_traj.getKnot();
  Eigen::MatrixXd ctrl_pts  = acc_traj.getControlPoint();
  int             dimension = ctrl_pts.cols();

  double acc = 0.0;
  for (int i = 0; i < ctrl_pts.rows(); ++i) {
    for (int j = 0; j < dimension; ++j) {
      acc += (times(i + 1) - times(i)) * ctrl_pts(i, j) * ctrl_pts(i, j);
    }
  }
