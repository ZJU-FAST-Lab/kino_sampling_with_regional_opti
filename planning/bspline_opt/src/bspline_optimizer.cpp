...
void BsplineOptimizer::setParam(const ros::NodeHandle& nh) {
  nh.param("bspline/lambda1", lambda1_, -1.0);
  nh.param("bspline/lambda2", lambda2_, -1.0);
  nh.param("bspline/lambda3", lambda3_, -1.0);
  nh.param("bspline/lambda4", lambda4_, -1.0);
  nh.param("bspline/lambda5", lambda5_, -1.0);
  nh.param("bspline/lambda6", lambda6_, -1.0);
  nh.param("bspline/lambda7", lambda7_, -1.0);
  nh.param("bspline/lambda8", lambda8_, -1.0);

  nh.param("bspline/dist0", dist0_, -1.0);
  nh.param("bspline/max_vel", max_vel_, -1.0);
  nh.param("bspline/max_acc", max_acc_, -1.0);
  nh.param("bspline/visib_min", visib_min_, -1.0);
  nh.param("bspline/dlmin", dlmin_, -1.0);
  nh.param("bspline/wnl", wnl_, -1.0);

  nh.param("bspline/max_iteration_num1", max_iteration_num_[0], -1);
  nh.param("bspline/max_iteration_num2", max_iteration_num_[1], -1);
  nh.param("bspline/max_iteration_num3", max_iteration_num_[2], -1);
  nh.param("bspline/max_iteration_num4", max_iteration_num_[3], -1);
  nh.param("bspline/max_iteration_time1", max_iteration_time_[0], -1.0);
  nh.param("bspline/max_iteration_time2", max_iteration_time_[1], -1.0);
  nh.param("bspline/max_iteration_time3", max_iteration_time_[2], -1.0);
  nh.param("bspline/max_iteration_time4", max_iteration_time_[3], -1.0);

  nh.param("bspline/algorithm1", algorithm1_, -1);
  nh.param("bspline/algorithm2", algorithm2_, -1);
  nh.param("bspline/order", order_, -1);
}

void BsplineOptimizer::setEnvironment(const tgk_planner::PosChecker::Ptr& env) {
