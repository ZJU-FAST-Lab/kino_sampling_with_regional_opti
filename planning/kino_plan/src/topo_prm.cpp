...
void TopologyPRM::init(const ros::NodeHandle& nh) {
  graph_.clear();
  ...
  resolution_ = pos_checker_ptr_->getResolution();
...
void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
                                ...
                                std::list<GraphNode::Ptr>& graph, vector<vector<Eigen::Vector3d>>& raw_paths,
...
std::list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
    ...
    pt = getSample();
    ++sample_num;
    if (pos_checker_ptr_->getVoxelState(pt) != 0) {
  ...
  std::cout << "[Topo]: node num before prune graph: " << graph_.size() << std::endl;
  // pruneGraph();
  std::cout << "[Topo]: node num after: " << graph_.size() << std::endl;
...
vector<GraphNode::Ptr> TopologyPRM::findVisibGuard(Eigen::Vector3d pt) {
  ...
  for (std::list<GraphNode::Ptr>::iterator iter = graph_.begin(); iter != graph_.end(); ++iter) {
    ...
    if (lineVisib(pt, (*iter)->pos_, resolution_, pc, 0, true)) {
...
bool TopologyPRM::lineVisib(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double thresh,
                            Eigen::Vector3d& pc, int caster_id, bool inflate) {
  return pos_checker_ptr_->checkLine(p1, p2, pc, inflate);
}
  ...
  if (graph_.size() > 2) {
    for (std::list<GraphNode::Ptr>::iterator iter1 = graph_.begin();
      ...
      if ((*iter1)->neighbors_.size() <= 1) {
        ...
        for (std::list<GraphNode::Ptr>::iterator iter2 = graph_.begin(); iter2 != graph_.end(); ++iter2) {
...
vector<vector<Eigen::Vector3d>> TopologyPRM::selectShortPaths(vector<vector<Eigen::Vector3d>>& input_paths,
                                                              int step) {
  ...
  vector<vector<Eigen::Vector3d>> paths = input_paths;
...
bool TopologyPRM::sameTopoPath(const vector<Eigen::Vector3d>& path1,
  ...
  for (int i = 0; i < pt_num; ++i) {
    if (!lineVisib(pts1[i], pts2[i], thresh, pc, 0, false)) {
      return false;
    }
  }
...
void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num) {
  ...
  for (int k = 0; k < iter_num; ++k) {
    ...
    for (int i = 1; i < dis_path.size(); ++i) {
      if (lineVisib(short_path.back(), dis_path[i], resolution_, colli_pt, path_id, true)) 
      {
        continue;
      }
      short_path.push_back(dis_path[i]);
    }
...
void TopologyPRM::shortcutPaths() {
  ...
  if (parallel_shortcut_) {
    ...
    for (int i = 0; i < raw_paths_.size(); ++i) {
      short_threads.push_back(std::thread(&TopologyPRM::shortcutPath, this, raw_paths_[i], i, 5));
    }
    for (int i = 0; i < raw_paths_.size(); ++i) {
      short_threads[i].join();
    }
  } else {
    for (int i = 0; i < raw_paths_.size(); ++i) shortcutPath(raw_paths_[i], i, 5);
  }
}
...
void TopologyPRM::setPoschecker(const PosChecker::Ptr& checker) { this->pos_checker_ptr_ = checker; }
